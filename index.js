...
function getSumRecurs(arr, result){
    return new Promise((resolve) => {
        if(!arr.length) return resolve(result);
        return resolve(Promise.all(arr.splice(0, 10).map(({id}) => processUser(id))).then(elements => {
            return getSumRecurs(arr, result += elements.reduce((acc, nextVal) => acc+= nextVal, 0))
        }));
    });
}
...
function startRecursive() {
    ...
    return getSumRecurs(users.slice(), 0);
